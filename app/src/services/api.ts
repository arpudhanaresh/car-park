...
export const parking = {
  getLayout: (start?: string, end?: string, floor: string = "Ground") => {
    let url = `/layout?floor=${encodeURIComponent(floor)}`;
    if (start && end) {
  ...
  getFloors: () => api.get<string[]>("/floors"),
  ...
  cancelBooking: (id: number, reason: string) =>
    api.delete(`/bookings/${id}`, { data: { cancellation_reason: reason } }),
  ...
  initiatePayment: (bookingId: number) =>
    api.post(`/payment/initiate/${bookingId}`),
  checkPaymentStatus: (bookingId: number, orderId: string) =>
    api.post(`/payment/check-status/${bookingId}?order_id=${orderId}`),
  downloadReceipt: (bookingId: number) =>
    api.get(`/bookings/${bookingId}/receipt`, { responseType: "blob" }),
};
...
export const admin = {
  ...
  updatePromo: (id: number, data: any) =>
    api.put(`/admin/promos/${id}/update`, data),
  ...
  calculateExitFee: (bookingId: number) =>
    api.post(`/admin/bookings/${bookingId}/calculate-exit`),
  completeBooking: (bookingId: number, data: any) =>
    api.post(`/admin/bookings/${bookingId}/complete`, data),
  notifyOverstay: (bookingId: number) =>
    api.post(`/admin/bookings/${bookingId}/notify-overstay`),
  processRefund: (bookingId: number) =>
    api.post(`/admin/bookings/${bookingId}/refund`),
};
